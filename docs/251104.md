...
```mermaid
gantt
    title Keep-Alive 타임라인 비교
    dateFormat YYYY-MM-DD HH:mm
    axisFormat %H:%M

    section Without Keep-Alive
    앱 실행     :active, 2025-11-04 10:00, 30m
    유휴 상태   :2025-11-04 10:30, 30m
    서버 자동 종료 :crit, 2025-11-04 11:00, 10m
    콜드스타트 :crit, 2025-11-04 11:00, 15m
    타임아웃! :crit, 2025-11-04 11:15, 5m
    ...
    서버 여전히 활성 :active, 2025-11-04 10:00, 120m
```
...
sequenceDiagram
    ...
    rect rgb(200, 220, 255)
        note over Frontend,Backend: 초기 세션 시작
        Frontend->>Backend: 1. /api/session/start
        Backend-->>Frontend: sessionId 반환
        Frontend->>KeepAlive: 2. useKeepAlive(true) 호출
        KeepAlive->>Monitor: 5초 대기 후 첫 핑 예약
    end

    rect rgb(220, 255, 220)
        note over KeepAlive,Backend: 25분 주기 Keep-Alive
        par 25분마다
            KeepAlive->>Backend: 3. GET /api/health (핑)
            Note over Backend: ⚡ < 5ms 응답
            Backend-->>KeepAlive: { status: 'ok', uptime: ... }
            KeepAlive->>Monitor: 4. 통계 기록 (success++)
        and 병렬 모니터링
            Monitor->>Monitor: 콘솔 출력:<br/>✅ Keep-alive ping successful
        end
    end

    rect rgb(255, 240, 220)
        note over KeepAlive,Backend: 만약 핑 실패 시 (네트워크 오류)
        KeepAlive->>Backend: ⚠️ Retry #1 (1초 대기 후)
        KeepAlive->>Backend: ⚠️ Retry #2 (2초 대기 후)
        KeepAlive->>Backend: ⚠️ Retry #3 (4초 대기 후)
        KeepAlive->>Monitor: 실패 기록 (failure++)
    end

    rect rgb(200, 255, 200)
        note over Frontend,Backend: 최종 결과
        Monitor->>Monitor: 통계: 50회 성공, 0회 실패<br/>성공률: 100%
    end
```
...
stateDiagram-v2
    ...
    Running --> Waiting: 5초 대기

    Waiting --> Pinging: 타임아웃
    Pinging --> Success: 응답 수신 (< 5ms)
    Pinging --> Failed: 오류 또는 타임아웃

    Success --> Waiting: 25분 대기
    Success --> Stopped: stop() 또는 세션 종료

    Failed --> Retry: attempt < maxRetries
    Retry --> Pinging: 지수백오프 재시도
    Retry --> Stopped: 최대 재시도 초과
...
```mermaid
graph TB
    subgraph Before["❌ N+1 Query 문제 (1200ms)"]
        A["GET /api/user/123"]
        A --> B["Query 1: User.findById<br/>시간: 300ms"]
        A --> C["Query 2: Preference.find<br/>시간: 500ms"]
        A --> D["Query 3: Setting.find<br/>시간: 400ms"]
        B --> E["Database"]
        C --> E
        D --> E
        E --> F["3개 쿼리 결과<br/>3회 네트워크 왕복"]
    end

    subgraph After["✅ Aggregation 해결 (300ms)"]
        G["GET /api/user/123"]
        G --> H["Aggregation Pipeline<br/>$match → $lookup → $lookup → $project"]
        H --> I["Database"]
        I --> J["1개 쿼리 결과<br/>1회 네트워크 왕복"]
    end
...
sequenceDiagram
    ...
    rect rgb(200, 220, 255)
        ...
        DB->>DB: userId='123'인 User 문서 필터링
        DB-->>Backend: 1개 사용자 문서
    end

    rect rgb(220, 240, 255)
        note over Backend,DB: Stage 2: $lookup (Preferences)
        DB->>DB: preferences 컬렉션<br/>조인 (User._id = Preference.userId)
        DB-->>Backend: User + preferences 배열
    end

    rect rgb(240, 220, 255)
        note over Backend,DB: Stage 3: $lookup (Settings)
        DB->>DB: settings 컬렉션<br/>조인 (User._id = Setting.userId)
        DB-->>Backend: User + preferences + settings
    end

    rect rgb(220, 255, 220)
        note over Backend,DB: Stage 4: $project
        DB->>DB: 불필요한 필드 제거<br/>필요한 필드만 선택
        DB-->>Backend: 최적화된 최종 결과
    end

    Backend-->>Client: { _id, name, email, preferences, settings }

    note over Client: 총 1개 쿼리, 1회 네트워크 왕복
```
...
```mermaid
graph LR
    subgraph Slow["❌ 느린 대시보드 (5-10초)"]
        S1["1. $match<br/>(userId)"]
        S2["2. $group<br/>(모든 필드)"]
        S3["3. sessionDates<br/>배열 누적"]
        S4["4. $lookup users<br/>(불필요)"]
        S5["5. $lookup analytics<br/>(불필요)"]
        S1 --> S2
        S2 --> S3
        S3 --> S4
        S4 --> S5
    ...
    subgraph Fast["✅ 빠른 대시보드 (500-1000ms)"]
        F1["1. $match<br/>(userId + 시간범위)"]
        F2["2. $group<br/>(필요한 필드만)"]
        F3["3. $addFields<br/>(계산 필드)"]
        F4["4. $project<br/>(필드 선택)"]
        F1 --> F2
        F2 --> F3
        F3 --> F4
    end

    Slow --> Improvement["개선 사항"]
    Fast --> Improvement
    Improvement --> I1["✅ $match에 시간 필터 추가<br/>스캔 범위 축소 (-1초)"]
    Improvement --> I2["✅ sessionDates 배열 제거<br/>메모리 사용 감소 (-1초)"]
    Improvement --> I3["✅ 불필요한 lookup 2개 제거<br/>I/O 작업 감소 (-2초)"]
    Improvement --> I4["✅ $project로 필요한 필드만<br/>네트워크 전송 감소 (-0.5초)"]

    I1 --> Result["⚡ 총 4.5초 단축<br/>90% 성능 개선"]
    I2 --> Result
    I3 --> Result
    I4 --> Result
...
```mermaid
timeline
    title 대시보드 요청 성능 비교 (GET /api/dashboard/summary)

    section 최적화 전 (5-10초)
        $match 필터링: 1초
        $group 계산: 1.5초
        sessionDates 배열 누적: 1초
        $lookup users: 1.5초
        $lookup analytics: 1.5초
        직렬화 및 전송: 0.5초

    section 최적화 후 (500-1000ms)
        $match 필터링 (최근 30일): 300ms
        $group 계산: 300ms
        $addFields 파생 필드: 100ms
        $project 필드 선택: 100ms
        직렬화 및 전송: 200ms

    section 개선율
        총 응답 시간: 7500ms → 1000ms
        개선율: 86% 단축
        병목 제거: 불필요한 lookup 2초 제거
        메모리 효율: 배열 누적 1초 제거
```
...
sequenceDiagram
    ...
    rect rgb(200, 220, 255)
        note over User,DB: 시나리오 1: 첫 번째 요청 (캐시 MISS)
        User->>Frontend: GET /api/user/preferences
        Frontend->>Cache: 캐시 확인
        Cache-->>Frontend: ❌ NOT FOUND
        Frontend->>DB: DB 쿼리
        Note over DB: 300ms 소비
        DB-->>Frontend: 데이터 반환
        Frontend->>Cache: 캐시에 저장 (TTL: 5분)
        Frontend-->>User: 응답 (300ms)
    end

    rect rgb(220, 255, 220)
        note over User,DB: 시나리오 2: 2번째 요청 (캐시 HIT, TTL 내)
        User->>Frontend: GET /api/user/preferences
        Frontend->>Cache: 캐시 확인
        Cache-->>Frontend: ✅ FOUND
        Frontend-->>User: 즉시 응답 (5ms)
        Note over Frontend: DB 쿼리 없음! ⚡
    end

    rect rgb(255, 240, 220)
        note over User,DB: 시나리오 3: TTL 만료 후 요청 (캐시 만료)
        Note over Cache: 5분 경과...
        User->>Frontend: GET /api/user/preferences
        Frontend->>Cache: 캐시 확인
        Cache-->>Frontend: ❌ EXPIRED
        Frontend->>DB: DB 쿼리
        DB-->>Frontend: 데이터 반환
        Frontend->>Cache: 캐시 갱신
        Frontend-->>User: 응답 (300ms)
    end

    rect rgb(255, 220, 220)
        note over User,DB: 시나리오 4: 데이터 업데이트 후 요청
        User->>Frontend: PUT /api/user/preferences
        Frontend->>DB: 업데이트
        DB-->>Frontend: 완료
        Frontend->>Cache: ❌ 캐시 무효화
        Frontend-->>User: 응답
        User->>Frontend: GET /api/user/preferences
        Frontend->>Cache: 캐시 확인
        Cache-->>Frontend: ❌ INVALIDATED
        Frontend->>DB: DB 쿼리
        DB-->>Frontend: 최신 데이터
        Frontend->>Cache: 새로운 캐시 저장
        Frontend-->>User: 응답 (300ms)
    end
```
...
```mermaid
graph TD
    A["API 요청<br/>/api/user/preferences"] --> B{Level 1<br/>브라우저 캐시}

    B -->|캐시 HIT| C["응답 0ms<br/>디스크에서 로드"]
    B -->|캐시 MISS| D{Level 2<br/>Backend 메모리<br/>node-cache}

    D -->|캐시 HIT| E["응답 5-10ms<br/>메모리에서 로드"]
    D -->|캐시 MISS| F{Level 3<br/>Database<br/>MongoDB}

    F -->|조회| G["응답 100-500ms<br/>DB 쿼리 실행"]

    G --> H["결과 캐싱<br/>Level 2에 저장"]
    H --> I["결과 반환<br/>Frontend"]
    I --> J["결과 캐싱<br/>Level 1에 저장"]
    J --> K["사용자 수신"]
...
```mermaid
stateDiagram-v2
    [*] --> Valid: 캐시 생성<br/>TTL 설정

    Valid --> Valid: GET 요청<br/>(캐시 HIT)

    Valid --> TTLExpired: 5분 경과

    TTLExpired --> [*]: 자동 삭제

    Valid --> InvalidatedByUpdate: PUT/POST 요청<br/>데이터 변경

    InvalidatedByUpdate --> [*]: 캐시 무효화

    Valid --> InvalidatedByDelete: DELETE 요청<br/>데이터 삭제

    InvalidatedByDelete --> [*]: 캐시 무효화
...
timeline
    ...
    section 즉시 (지금)
        Render Pro 업그레이드: 5분, $7/월

    section 이번 주 (Phase 1)
        Keep-Alive 구현: 1시간, 무료
        데이터베이스 인덱스: 30분, 무료
    ...
    section 1개월 후 (Phase 3, 선택)
        Redis 추가: $15/월
        CDN 구성: 무료~$5/월
        모니터링 개선

    section 3개월 후 (최종)
        Railway 검토: 마이그레이션 옵션
        성능 벤치마크
        비용 최적화
```
