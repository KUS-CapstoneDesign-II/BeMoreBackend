...
// 감정 타입 (8가지 - MediaPipe 표준)
type EmotionType = 'anxious' | 'sad' | 'angry' | 'happy' | 'neutral' | 'fearful' | 'disgusted' | 'surprised';
...
| **타임아웃** | 30초 |
| **대화 히스토리** | 최근 10개 메시지 |
| **감정 타입** | 8가지 (anxious, sad, angry, happy, neutral, fearful, disgusted, surprised) |
...
백엔드는 **MediaPipe 표준 8가지 감정**을 모두 지원합니다:
...
| 두려움, 공포 | `fearful` | 안전감 제공, 두려움 완화 |
| 혐오, 불쾌감, 거부감 | `disgusted` | 불쾌감 이해, 상황 재구성 |
| 놀람, 충격 | `surprised` | 놀람 수용, 상황 정리 |
| 중립, 평온, 기타 | `neutral` | 일반 상담 |

**✅ 프론트엔드 작업 불필요**:
- MediaPipe의 8가지 감정을 그대로 백엔드로 전송 가능
- 별도의 감정 매핑 로직 구현 불필요
- 각 감정에 맞춤형 AI 상담 프롬프트 적용됨
